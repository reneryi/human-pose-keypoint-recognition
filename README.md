...
1. 在侧边栏切换到“视频关键点识别”。
2. 上传 `mp4`、`avi`、`mov` 或 `mkv` 视频文件。
3. 点击“开始识别视频关键点”。
4. 程序逐帧调用 MediaPipe Pose 检测人体关键点。
5. 使用 OpenCV 将关键点和骨架绘制到每一帧。
6. 结果视频保存到 `outputs/video_result/`，同时支持在页面中预览和下载。
